...
#
# Exam: 02. Find Authors' Nationalities
#
def find_authors_nationalities() -> str:
    authors_with_nationality = Author.objects.filter(nationality__isnull=False)
    # authors_with_nationality = Author.objects.exclude(nationality=None)

    return '\n'.join(f'{str(a)} is {a.nationality}' for a in authors_with_nationality)
