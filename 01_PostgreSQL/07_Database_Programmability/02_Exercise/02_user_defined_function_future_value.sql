...
CREATE OR REPLACE FUNCTION fn_calculate_future_value(
    IN initial_sum FLOAT,
    IN yearly_interest_rate FLOAT,
    IN number_of_years FLOAT,
    OUT future_value NUMERIC
) AS
$$
BEGIN
    future_value := TRUNC((initial_sum * ((1 + yearly_interest_rate) ^ number_of_years))::numeric, 4);
END
$$
    LANGUAGE plpgsql;
